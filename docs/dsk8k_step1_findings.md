...
> **状态：** Step 1.1 + 1.2 已在生产数据上完成验证；1.3（跨时间稳定性）待数据到位。
> **任何优化决策必须等 1.3 验证完成后才能启动。**
>
> **创建：** 2026-04-30
> **数据来源：** `data/deepseek_v3.1_8k/raw/DeepSeek-V3.1-Terminus-NoThinking-8K.csv`（生产采样，2 小时窗口）
...
→ 之前"chain 占 87.5% 上下文"的字节级表述应修正为 **"token 级约 25–37% prefill 可被 cache"**（按 8K context 算）。
...
| 跨租户共享 | ❌ 已确认无跨租户共享 |
| Pin 56 blocks 的覆盖率 | 41.2% of total = 46.6% of heavy user |
| 单 chain pin TTFT 节省估算 | 主流量约 25–37% prefill saved |

**结论：在 1.3 验证之前，DS-8K 优化方向"暂时可行但未确认"**，禁止启动 Step 3 算法实现。
